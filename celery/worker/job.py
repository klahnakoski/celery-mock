...
class Request(object):
    ...
    def revoked(self):
        ...
        if self.id in revoked_tasks:
            Log.info('Discarding revoked task: %s[%s]', self.name, self.id)
    ...
    def on_success(self, ret_value, now=None, nowfun=monotonic):
        ...
        if _does_info:
            now = now or nowfun()
            runtime = self.time_start and (now - self.time_start) or 0
            Log.info("Task {{name}}s[{{id}}s] succeeded in {{runtime}}ss: {{return_value}}s".strip(), {
    ...
    def on_retry(self, exc_info):
        ...
        if _does_info:
            Log.info("Task {{name}}s[{{id}}s] {{exc}}s".strip(),
                 {'id': self.id, 'name': self.name,
    ...
    def _log_error(self, einfo, send_failed_event=True):
        ...
        format = "Task {{name}}s[{{id}}s] {{description}}s: {{exc}}s"
        if internal:
            if isinstance(einfo.exception, MemoryError):
                ...
                format = "Task {{name}}s[{{id}}s] {{description}}s"
                description = 'ignored'
